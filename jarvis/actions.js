...
async function proposeAction(priorityId) {
...
async function executeApprovedAction(approvalId) {
  ...
  if (req.expires_at && new Date() > new Date(req.expires_at)) {
    await queryDb("UPDATE jarvis_approval_requests SET status = 'expired', updated_at = now() WHERE id = $1;", [approvalId]);
  ...
  if (req.action_type === 'draft_email_proposal') {
    ...
    try {
      ...
      throw new Error(`Failed to queue Hermes dry-run job: ${err.message}`);
    }
  } else {
  ...
  await queryDb(
    `UPDATE jarvis_approval_requests 
     SET executed_at = now(), status = 'executed', execution_result = $1, updated_at = now() 
     WHERE id = $2;`,
    [JSON.stringify({ status: 'success', output: outputText }), approvalId]
  );
